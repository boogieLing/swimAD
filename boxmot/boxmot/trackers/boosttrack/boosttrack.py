...
from boxmot.boxmot.trackers.boosttrack.assoc import (
    associate,
    iou_batch,
    MhDist_similarity,
    shape_similarity,
...
from boxmot.boxmot.trackers.boosttrack.kalmanfilter import KalmanFilter
from boxmot.boxmot.trackers.boosttrack.ecc import ECC
    ...
    def update(self, dets: np.ndarray, img: np.ndarray, embs: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        if self.with_reid and len(self.trackers) > 0:
            ...
            if dets_embs.shape[0] == 0:
                ...
                emb_cost = dets_embs.reshape(dets_embs.shape[0], -1) @ tracker_embs.reshape(
                    (tracker_embs.shape[0], -1)).T
        else:
            emb_cost = None
