...
from boxmot.boxmot.multiview_tool.grid_determine import find_position_between_lines
import random 
random.seed(42)
...
class View(object):
    ...
    def set_main_lines(self):
        ...
        num_grid_width = 8
        num_grid_height = 22
        left_up, left_bottom, right_up, right_bottom = (0, 0), (0, height), (width, 0), (width, height)
        width_grid = np.linspace(0, width, num_grid_width+1)
        height_grid = np.linspace(0, height, num_grid_height+1)

        top_border = [(x, 0) for x in width_grid]
        bottom_border = [(x, height) for x in width_grid]
        left_border = [(0, x) for x in height_grid]
        right_border = [(width, x) for x in height_grid]
...
class AssociationData(object):
    ...
    def LabelData2AssociationData(self, image_data, view):
        ...
        for label in image_data.labels.objects:
            object_id, cx, cy, cls_id = label[0], label[1] + label[3]/2.0, label[2] + label[4]/2.0, label[5]
            point = Point(cx, cy, cls_id, view)
            grid = point.grid
    ...
    def associate(self, views_projections):
        ...
        cost_matrix = np.full((len(main_view_point), len(associate_view_point)), 1e9)
