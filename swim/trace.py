...
class MultiViewAssociationStream(object):
    def __init__(self):
        self.main_view = None
        self.grid_root = "region_calibration_data_v2.json"
        self.init_views()
    ...
    def init_views(self):
        ...
        self.main_view = View(
            MAIN_VIEW, p1=0, p2=0, k1=0, k2=0, k3=0, fx=1280, fy=720, cx=1280, cy=720,
            fx_ratio=1.0, fy_ratio=1.0, grid_info=None,
        )
        view1 = View(
            '1', p1=0, p2=0, k1=-0.5353, k2=0.2875, k3=-0.0906,
            fx=1621.9, fy=1856.1, cx=1116.3, cy=742.9178, fx_ratio=1.33, fy_ratio=1.33,
            grid_info=grid_info[0],
        )
        view2 = View(
            '2', p1=0, p2=0, k1=-0.5153, k2=0.2845, k3=-0.0906,
            fx=1621.9, fy=1856.1, cx=1116.3, cy=742.9178, fx_ratio=1.34, fy_ratio=1.34,
            grid_info=grid_info[1],
        )
        view3 = View(
            '3', p1=0, p2=0, k1=-0.5253, k2=0.2845, k3=-0.0906,
            fx=1621.9, fy=1856.1, cx=1116.3, cy=742.9178, fx_ratio=1.34, fy_ratio=1.34,
            grid_info=grid_info[2],
        )
        view4 = View(
            '4', p1=0, p2=0, k1=-0.5253, k2=0.2875, k3=-0.0906,
            fx=1621.9, fy=1856.1, cx=1116.3, cy=742.9178, fx_ratio=1.34, fy_ratio=1.34,
            grid_info=grid_info[3],
        )
    ...
    def init_view_association(self):
        ...
        for view in self.views:
            views_projections[f"({MAIN_VIEW},{view.name})"] = ViewAssociation(target_view=self.main_view,
                                                                              source_view=view)
...
class View(object):
    def __init__(
            self, name, p1=0.0, p2=0.0, k1=0.0, k2=0.0, k3=0.0, fx=0.0, fy=0.0, cx=0.0, cy=0.0,
            fx_ratio=1.0, fy_ratio=1.0, grid_info=None,
    ):
        '''
        input:
...
class ViewAssociation(object):
    def __init__(self, target_view, source_view):
        ...
        '''
        # four corners
        target_points = [[MARGIN_WIDTH, MARGIN_HEIGHT], [MARGIN_WIDTH, MARGIN_HEIGHT + POOL_HEIGHT],
                         [MARGIN_WIDTH + POOL_WIDTH, MARGIN_HEIGHT],
                         [MARGIN_WIDTH + POOL_WIDTH, MARGIN_HEIGHT + POOL_HEIGHT]]
    ...
    def box_projection(self, box):
        ...
        return Box(new_x1, new_y1, new_x2, new_y2, box.conf, box.cls_id, self.target_view, box.source_view,
                   box.image_path, is_distorted=box.is_distorted)
    ...
    def LabelData2AssociationData(self, image_data):
        ...
        for label in image_data.labels.objects:
            ...
            box = Box(x1, y1, x2, y2, conf, cls_id, view, view, image_data.image_path,
                      is_distorted=True)  # used for test
...
class Point(object):
    ...
    def anti_distortion(self):
        ...
        if self.is_distorted:
            ...
            undistorted_point = cv2.undistortPoints(point, self.view.camera_matrix, self.view.dist_coeffs,
                                                    P=self.view.new_camera_matrix)
...
class Box(object):
    ...
    def anti_distortion(self):
        if self.is_distorted:
            ...
            undistorted_point1 = cv2.undistortPoints(point1, self.view.camera_matrix, self.view.dist_coeffs,
                                                     P=self.view.new_camera_matrix)
            self.x1, self.y1 = undistorted_point1[0][0]

            point2 = np.array([[self.x2, self.y2]], dtype=np.float32)
            undistorted_point2 = cv2.undistortPoints(point2, self.view.camera_matrix, self.view.dist_coeffs,
                                                     P=self.view.new_camera_matrix)
            self.x2, self.y2 = undistorted_point2[0][0]

            pointc = np.array([[self.cx, self.cy]], dtype=np.float32)
            undistorted_pointc = cv2.undistortPoints(pointc, self.view.camera_matrix, self.view.dist_coeffs,
                                                     P=self.view.new_camera_matrix)
