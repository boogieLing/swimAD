...
from constant import TRACKER_LOG_PATH
from entries.track_entry import TrackObservation, DetectionEntry
...
class DualChannelLogger(threading.Thread):
        ...
        with open(self.log_file, "a", encoding="utf-8") as f:
            while self.running:
                try:
                    ...
                    print(f'DualChannelLogger: {type(e).__name__}: {e}')
    ...
    @staticmethod
    def _print_to_console(item: dict):
...
class TrackWindowManager:
    ...
    def __init__(self, max_window_size: int = 30, ttl: int = 10, save_dir: str = TRACKER_LOG_PATH,
                 snapshot_interval: int = 60, frame_window_span: Optional[int] = None):
        self.windows: Dict[str, TrackWindow] = {}
        self.max_window_size = max_window_size
        self.ttl = ttl  # 存活时长，秒
        self.save_dir = Path(save_dir)
        self.logger = DualChannelLogger(self.save_dir)  # 异步双通道日志记录器
        self.logger.start()
        self.show_logger = setup_logger(self.__class__.__name__)  # 用于显示日志
    ...
    def shutdown(self, timeout: int = 5):
        ...
        """
        self.show_logger.info("[TrackWindowManager] Initiating graceful shutdown...")

        # 1. 停止异步日志线程
        if hasattr(self, "logger") and self.logger is not None:
            self.logger.stop()
            self.logger.join(timeout=timeout)
            self.show_logger.warning("[TrackWindowManager] Logger thread stopped.")
        ...
        if hasattr(self, "snapshot_scheduler") and self.snapshot_scheduler is not None:
            ...
            self.show_logger.warning("[TrackWindowManager] Snapshot scheduler stopped.")

        # 3. 退出前再保存一次完整快照（保险起见）
        final_path = self.save_snapshot()
        self.show_logger.info(f"[TrackWindowManager] Final snapshot saved to {final_path}")

        self.show_logger.info("[TrackWindowManager] Shutdown complete.")
    ...
    def detect_abnormal_tracks(
            self, hook_fn: Callable[[str, List[TrackObservation]], Optional[Dict]]
    ) -> Dict[str, Dict]:
        """
    ...
    @staticmethod
    def format_rule_value(value) -> str:
        """
